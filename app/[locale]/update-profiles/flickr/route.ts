...
export const GET = generateUpdateProfileHandler({
  ...
  generateProfileInfo: async () => {
    if (!serverEnv.FLICKR_API_KEY || !serverEnv.FLICKR_USER_ID) {
      throw new Error("FLICKR_API_KEY and FLICKR_USER_ID are required");
    }

    const data = await fetchJson(
      `https://api.flickr.com/services/rest/?method=flickr.people.getPublicPhotos&api_key=${serverEnv.FLICKR_API_KEY}&user_id=${serverEnv.FLICKR_USER_ID}&extras=description%2C+license%2C+date_upload%2C+date_taken%2C+owner_name%2C+icon_server%2C+original_format%2C+last_update%2C+geo%2C+tags%2C+machine_tags%2C+o_dims%2C+views%2C+media%2C+path_alias%2C+url_sq%2C+url_t%2C+url_s%2C+url_q%2C+url_m%2C+url_n%2C+url_z%2C+url_c%2C+url_l%2C+url_o&per_page=500&format=json&nojsoncallback=1`,
      z.object({
        photos: z.object({
          total: z.number(),
          photo: z.array(
