...
This approach is used when it is easier to scrape a web page than to interact with a third-party API.

Profiles are updated on a schedule (see [Deployment](#deployment) section).
Because `/update-profiles/*` endpoints are public, a security token is introduced to prevent unauthorised usage.
...
To be able to update profile infos locally, you will need to set `UPDATE_PROFILE_SECURITY_TOKEN` in `.env.local`.
