...
It represents a simple instance of a&nbsp;Next.js app with server components, route handlers and domain-based internationalization.
Feel free to explore this repository to learn something new or even to reuse its code in your own projects.

## Key ingredients

- **[Next.js](https://nextjs.org)** (App Router with [Cache Components](https://nextjs.org/docs/app/getting-started/cache-components)) as the architecture framework
- **[React](https://react.dev)** to define the UI (server and client components)
...
- **[Playwright](https://playwright.dev)** to scrape profile infos from third-party websites
- **[Zod](https://zod.dev)** to validate environment variables and scraped data
...
- **[Husky](https://typicode.github.io/husky/)** and **[lint-staged](https://github.com/lint-staged/lint-staged)** to format files before they are committed
...
Because each locale is served from its own domain (see [Playing with i18n](#playing-with-i18n)), the whole app lives under a single `[locale]` segment, which [`proxy.ts`](proxy.ts) picks based on the request host.
Pages and route handlers are kept next to the components and helpers they use:

- [`app/[locale]/layout.tsx`](app/[locale]/layout.tsx) — the shell shared by all pages, with its parts in `layout/`
- [`app/[locale]/page.tsx`](app/[locale]/page.tsx) — the home page, with its parts in `page/`
- [`app/[locale]/photos/`](app/[locale]/photos/) — a secondary page
- [`app/[locale]/update-profiles/`](app/[locale]/update-profiles/) — one route handler per profile
- [`app/[locale]/shared/`](app/[locale]/shared/) — bits that are used in more than one place

The remaining directories are [`i18n/`](i18n/) (next-intl setup), [`messages/`](messages/) (translated messages), [`public/`](public/) (static assets) and [`k8s/`](k8s/) (deployment manifests).
...
This simplifies the architecture and reduces the amount of JavaScript that browsers have to download.

Profile infos are updated inside Next.js route handlers.
...
On top of that, a [Husky](https://typicode.github.io/husky/) pre-commit hook runs [lint-staged](https://github.com/lint-staged/lint-staged), which formats staged files with Prettier.
Other Husky hooks run `pnpm install` after `git checkout`, `git merge` and `git rebase`, which keeps `node_modules` in sync with `pnpm-lock.yaml`.
...
Next.js apps are often deployed to cloud-native environments such as [Vercel](https://vercel.com), [Netlify](https://www.netlify.com), [AWS Amplify](https://aws.amazon.com/amplify/), etc.
This makes them highly scalable and resilient to failures.
One limitation that cloud-native deployments impose on Next.js apps is to do with the size of the [Lambda functions](https://en.wikipedia.org/wiki/AWS_Lambda).
...
These yamls can serve as examples for deploying similar Next.js apps with ‘heavy’ route handlers.
Such handlers would be slow inside Lambda functions or even exceed [their size limit](https://vercel.com/docs/functions/limitations).
...
This was before Next.js implemented [API routes](https://nextjs.org/docs/pages/building-your-application/routing/api-routes), so I had to split the app into two microservices: `frontend` and `graphql-server`.
...
When Next.js [announced](https://nextjs.org/blog/next-13) the new `/app` directory in late 2022, I saw this as an opportunity to refactor my mini-website and to experiment with [React server components](https://nextjs.org/docs/app/getting-started/server-and-client-components).
